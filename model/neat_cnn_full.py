...
import numpy as np
from model.layers import AttentionCnnBlock
...
class NeatCnnFull(nn.Module):
    def __init__(self, vocab_size, user_size, user_embed_size, cluster_size, num_classes, max_post_len, config):
        super(NeatCnnFull, self).__init__()
        self.config = config
        ...
        # 3 conv
        self.conv1 = AttentionCnnBlock(self.config.embed_size, self.config.kernel_size[0], self.config.num_channels,
                                       max_post_len, cluster_size)
        self.conv2 = AttentionCnnBlock(self.config.embed_size, self.config.kernel_size[1], self.config.num_channels,
                                       max_post_len, cluster_size)
        self.conv3 = AttentionCnnBlock(self.config.embed_size, self.config.kernel_size[2], self.config.num_channels,
                                       max_post_len, cluster_size)
    ...
    def forward(self, x_post_word_idxs, x_user_idxs, x_user_clusters):
        ...
        conv_out1, attn1 = self.conv1(embedded_post_word_idxs, x_user_clusters)
        conv_out1 = conv_out1.squeeze(2)
        conv_out2, attn2 = self.conv2(embedded_post_word_idxs, x_user_clusters)
        conv_out2 = conv_out2.squeeze(2)
        conv_out3, attn3 = self.conv3(embedded_post_word_idxs, x_user_clusters)
        conv_out3 = conv_out3.squeeze(2)
        ...
        # Concat user expertise and post feature
        ...
        return final_out, scaled_user_creds, [(attn1, self.config.kernel_size[0]),
                                              (attn2, self.config.kernel_size[1]),
                                              (attn3, self.config.kernel_size[2])]

    @staticmethod
    def compile_attentions(attentions, max_seq_len):
        kernel_post_idx_attn = []
        for attn, kernel_size in attentions:
            attn_cpu = attn.detach().cpu().numpy()
            post_idx_attn = []
            for post_attn in attn_cpu:
                idx_attn = np.zeros(max_seq_len)
                start_idx = 0
                for i, conv_attn in enumerate(post_attn):
                    for j in range(start_idx, start_idx + kernel_size):
                        idx_attn[i + j] += conv_attn
                post_idx_attn.append(idx_attn)
            kernel_post_idx_attn.append(np.vstack(post_idx_attn))
        return np.mean(kernel_post_idx_attn, axis=0)
