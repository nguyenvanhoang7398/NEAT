...
class NeatWPEU(nn.Module):
    def __init__(self, vocab_size, user_size, user_embed_size, num_classes, config):
        super(NeatWPEU, self).__init__()
        self.config = config
        ...
        post_feature_size = self.config.hidden_size * self.config.n_layers * (1 + self.config.bidirectional)
        self.fc = nn.Linear(
